...
''' % (self.__getExportFile__(), self.sourceBaseId, self.sourceDataFile.getPackage())
    ...
    def addNewRegressionResultRecord(self, releaseId, fileId, status):
        recordLine = "null, %s, %s, %s, 0, null, null, null" % (releaseId, fileId, str(status))
...
def main():
    ...
    for currentFile in dataFiles:
        ...
        for cfLine in cf:
            ...
            (cfFileName, cfStatus) = cfLine.split(',')[:2]
            ...
            if recordId != -1:
                ...
                dbutil.addNewRegressionResultRecord(release_id, recordId, str(statusTotals.getNumericValue(cfStatus)))
            else:
        ...
        for newLine in newRecordLines:
            (cfFileName, cfStatus) = newLine.split(",")[:2]
            justDirName = os.path.dirname(cfFileName)
            justFileName = os.path.basename(cfFileName)
